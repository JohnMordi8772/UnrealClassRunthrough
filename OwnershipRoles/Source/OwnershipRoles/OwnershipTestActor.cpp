...
void AOwnershipTestActor::Tick(float DeltaTime)
{
	...
	//Draw a debug sphere to help visualize the ownership radius
	DrawDebugSphere(GetWorld(), GetActorLocation(), OwnershipRadius, 32, FColor::Yellow);

	//Get the closest AOwnershipRolesCharacter within thet ownership radius
	//If it's different from the current one, set it as the owner
	if (HasAuthority())
	{
		...
		const FString Values =
			FString::Printf(TEXT("Jack Mordi\nLocalRole = %s\nRemoteRole = %s\nOwner = %s\nConnection = %s"),
